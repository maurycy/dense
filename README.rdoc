...
In most Ruby on Rails application, data serialization is not an issue.
...
To set the objective for all #is_a? objects:
	...
	user = User.new
	Dense.unpack(Dense.pack(user)) == user

If there are few classes you'd like to match, specify an array.

To set the objective for all #respond_to? objects:
...
It is generally not recommended to set an objective for base classes, such as Array, Hash etc.

To set the objective dynamically:

	Dense.objective(:compact) do |o|
		o.proc   {|v| v.respond_to?(:size) && v.size >= 128 } 
		o.pack   {|v| Marshal.dump(v) }
		o.unpack {|v| Marshal.load(v) }
	end

For the performance reasons, it is not recommended to overload #proc, and, if possible, to hedge it with #is_a? matching:
...
If you are not sure which objective is defined for a given object:

	Dense.which(%w(a b c)) == :default
